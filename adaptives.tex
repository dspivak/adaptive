...
$$[p,q] = \sum_{\phi : p \to q} y^{\sum_{I \in p(1)} q[\phi_1(I)]}.$$
\end{definition}

Using \cite[Definition 2.10]{Learners} applied to $[p,q]$, we get the following definition of $[p,q]$-coalgebras.
...
When, for each $s \in S$, $\beta_s$ is the constant function sending everything to $s$, we say the coalgebra $\S$ has \emph{no updates}, as no output $I \in p(1)$ nor input from $q[\beta_0(s)_1(I)]$ can produce a different state from $s$.

\begin{example}
A special case of $[p,q]$-coalgebras with no updates are given by maps $\phi \in \poly(p,q)$. For each such $\phi$, there is a coalgebra $\S_\phi$ with a singleton set of states, $\beta_0$ sending the point to $\phi$, and (by necessity) no updates.
\end{example}
...
\begin{proposition}
The categories $[p,q]\coalg$ form the categories of morphisms in a bicategory $\org$ with polynomials as objects.
\end{proposition}

It is no accident that $\org$ denotes both this bicategory and the categorical operad in \cite[Definition 2.19]{Learners}, as both are derived from the monoidal double category $\org$ described in the following sections. For now, we merely present the horizontal identities and composites in this bicategory.
...
where the first functor is described in \cite[Proposition 2.13]{Learners}, and the second is given by the fact that $(-)\coalg$ is a functor $\poly \to \smcat$, using a map of polynomials $[p,q] \otimes [q,r] \to [p,r]$. On positions, this map takes the form
...
Concretely, the composite of a $[p,q]$-coalgebra $\S$ and a $[q,r]$-coalgebra $\S'$ is a $[p,r]$-coalgebra $\S;\S'$ is described as follows:
\begin{itemize}
	\item its states are given by $S \times S'$
...
\cite[Proposition 2.13]{Learners} shows that the tensor product $\otimes$ of polynomials extends to make $\org$ a monoidal bicategory. In particular, for polynomials $p,q,p',q'$ there is a functor
...
While defining $\org$ as a monoidal bicategory is sufficient for most of the constructions of $\org$-enriched structures in the next chapter, distinguishing the maps of polynomials as a special class of coalgebras allows $\org$ to encode all of $\poly$ and also facilitates a good definition of morphism between adaptives. 

Specifically, the definition of $\org$ as a monoidal bicategory extends to a monoidal (pseudo-)double category with coalgebras as horizontal morphisms, maps in $\poly$ as vertical morphisms, and squares as below given by maps of coalgebras from $\S_\psi;\S$ to $\S';\S_\phi$. 
\[\begin{tikzcd}
...
The definition of morphisms of adaptives comes from morphisms of enriched monoidal categories as defined in \cite{ShapiroEnrichment}. %cite specifically
...
\begin{itemize}
	...
	\item The compositors $\S_{\ell,m};\S_{m,n} \to \S_{\ell,n}$ send the pair $((L,f,p),(M,g,q))$ to 
...
This is proven as \cref{gradientproof} in \cref{coherenceproofs}.
...
The equations governing the structure maps in an adaptive are as follows:
\begin{itemize}
...
\[\hspace{-1.8cm}\begin{tikzcd}[column sep={60,between origins}]
...
\arrow[Rightarrow,shorten=5,from=W,to=X]
\end{tikzcd}\]
	\item The compositor left unit law
	\item The compositor right unit law
...
p^{\otimes (m + m' + m'')} \rar[slash, ""{name=V, above},swap]{\S_{m+m'+m'',n+n'+n''}} & p^{\otimes (m + m' + m'')}
\arrow[Rightarrow,shorten=5,from=S,to=T]
\arrow[Rightarrow,shorten=5,from=U,to=V]
...
\arrow[Rightarrow,shorten=5,from=U,to=V]
\end{tikzcd}\]
	\item The productor left unit law
	\item The productor right unit law
\end{itemize}
\end{definition}

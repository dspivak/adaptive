...
Organisms don't defy the physics in which they are implemented. But the term \emph{physics} can be used to refer to much more than simply the fundamental building blocks of reality; its laws may themselves be organized. There may be patterns of construction that make sense at all levels, patterns like attraction, entropic ascent, or even birth and death. If patterns exist at all levels, from atoms to stars, then one may say they are even \emph{more real} than patterns that exist at only the smallest levels.

Organized matter can exploit self-similarity in the patterns of nature. One often sees plants with fractal-like symmetries that hold across hierarchical levels. Microcosm principles in mathematics also show that to even display certain structures, a system must be embedded in a world with even higher-level versions of that very structure \cite{baez1997quantum}.

...\\

If organisms are built to depend on  are co-creators of physics.

Indeed the laws of nature referred to the totality of how nature is put together and changes through time, and not just to any sort of atomic bedrock at a purportedly bottom layer. The concept of \emph{emergence} is often used to rescue a bottom-up interpretation of physics from the experimental data showing that higher-level structures have downward causality on the patterns that arise in lower-level parts \cite{laughlin2005different}. Rather than seek the building blocks of reality and then stand with conceptual crutches on them, we propose to seek the construction of reality and forgo the need for a bottom-level foundation.
...
Using \cite[Definition 2.10]{spivak2021learners} applied to $[p,q]$, we get the following definition of $[p,q]$-coalgebras.
